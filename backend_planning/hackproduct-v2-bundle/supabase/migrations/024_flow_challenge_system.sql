...
CREATE TABLE challenge_attempts_v2 (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_challenges_taxonomy ON challenges(paradigm, industry, difficulty) WHERE is_published = true;
CREATE INDEX idx_challenges_roles ON challenges USING GIN(relevant_roles) WHERE is_published = true;
CREATE INDEX idx_challenges_companies ON challenges USING GIN(company_tags) WHERE is_published = true;
CREATE INDEX idx_challenges_published ON challenges(is_published, difficulty) WHERE is_published = true;
CREATE INDEX idx_flow_steps_challenge ON flow_steps(challenge_id, step_order);
CREATE INDEX idx_step_questions_step ON step_questions(flow_step_id, sequence);
CREATE INDEX idx_flow_options_question ON flow_options(question_id, option_label);
CREATE INDEX idx_attempts_v2_user ON challenge_attempts_v2(user_id, created_at DESC);
CREATE INDEX idx_attempts_v2_status ON challenge_attempts_v2(user_id, status) WHERE status = 'in_progress';
CREATE INDEX idx_step_attempts_attempt ON step_attempts(attempt_id, step);
CREATE INDEX idx_competencies_user ON learner_competencies(user_id);
CREATE INDEX idx_luma_context_v2_user ON luma_context_v2(user_id, is_active, created_at DESC);
