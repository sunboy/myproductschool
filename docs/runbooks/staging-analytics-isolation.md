...
## Failed artifact recovery canary

The one authorized recovery run against READY deployment `dpl_4gzMyq6K7V9xeAxaS5C8qHfJECpS` at commit `7e3c4d2822abee64560fc9db3d12734729618457` used run `03b54869-1c6a-415a-a253-03c2f670b2a1` and session `7ef76e1f-6de6-4f97-8638-3c34126b6b9a`. It restored the original workspace and verified the six prior passing checkpoints, then the skill-first Claude command exited 1 at `__CANARY_RESUME_SKILL_DONE_03b54869-1c6a-415a-a253-03c2f670b2a1___exit_1`. The bounded state contains no deeper safe CLI cause. No new skill or report was verified, and no new checkpoint assessment or final grade was produced.

The failure-cleanup step captured an early provider spend snapshot of `$0.09948`. The preserved terminal state later recorded a gateway snapshot to `spentUsd=0.1754103` and `observedSpendCents=18`; use that later snapshot as the new session's cost. Its separate `recordedSpendCents=49` is an inherited session accounting baseline: start replaces the session ID through an attempt-ID upsert, while fresh provisioning state did not reset recorded spend. This can suppress new-session usage increments and is a confirmed accounting defect, not a second charge. Commit `67dde19d` resets observed and recorded spend to zero for the new session/key; 6 focused tests and TypeScript pass. It preserves existing usage events but does not recover late prior-session spend after row replacement. No paid canary was rerun for this correction. The original source session's six assessments and `$0.4924225` raw spend remain separate evidence.

Cleanup finished as `failed_cleanup_complete`: the owned session was reaped, its revision was absent, its exact key was blocked and retained, and its TEST subscription was canceled. The signed deletion webhook processed once with cleared processing lease and token, zero pending webhooks, and no last error. Profile, hosted plan, and subscription returned to Free with analytics and Pro access false and zero blocking subscriptions. Preserve the mode-0600 state `.vercel/staging-analytics-canary-state-resume-7e3c4d28.json` (21,926 bytes, SHA-256 `7b57622b328fcb035e21dd74c914466956f3c62061b79bf783f390c77c0428ff`). No rerun is authorized by this evidence.
