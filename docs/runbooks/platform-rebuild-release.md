...
## Current release gate update — September 6, 2026

Candidate `471b7ada1ecad6ec1c45338f81b16e6a3c277ab7` is READY as `dpl_ECUyVi6Rh18RnXrwciLAm971HfRF`. The clean genuine Stripe TEST subscription lifecycle now passes, including duplicate and delayed delivery and cancellation restoring Free access. The endpoint remains enabled; the canary subscription is canceled. Browser Checkout completion and portal interaction remain open. See `staging-billing-lifecycle.md` for exact events and the separately preserved earlier failure.

Staging source history now includes 181 migrations through `20260906160000_scope_catalog_admin_policies.sql`. The new catalog policy repair keeps published reads public while limiting admin policy evaluation to authenticated clients. This is a staging-applied change; production migration and release remain separate decisions.

Review of the pending analytics canary found that the existing key-revocation helper has no application callers. Sandbox teardown alone does not prove immediate gateway-key revocation. The next application candidate must block ended sessions' exact keys while retaining their spend records for reconciliation; the canary must verify application behavior rather than count manual harness cleanup as success. Complete analytics finalization remains open. No learner session was started during this review.
