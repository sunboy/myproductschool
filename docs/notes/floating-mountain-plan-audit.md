...
| 5. Plan limits + throttle | Partial | Paywall N2 coverage passed. User-triggered grading paths now reserve `ai_grading_runs` and pass AI spend budgets for FLOW elaboration/freeform submit, coding submit/finalize, interview challenge submit, and interview-loop debrief; duplicate FLOW step retries return saved answers before quota reservation. Full live per-route AI throttle/limit E2E across every AI route is still not recorded. |
