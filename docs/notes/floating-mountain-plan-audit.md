...
| 24. Markdown rendering | Partial | Challenge scenario/context prose now renders markdown in `CaseContextPane`, `ChallengeWorkspace`, and `FlowWorkspace` while preserving inherited text color via `Md tone="inherit"`. Evidence: commit `8d31e8a`, `npx vitest run tests/components/Md.test.tsx` passed `5/5`, `npx tsc --noEmit --pretty false`, `npm run lint`, `npm run build`, and `npm run secrets:scan` passed. Full raw-prose replacement across every app surface and Hatch challenge-link E2E are still not recorded. |
