...
| --- | --- | --- |
| 1. Build, lint, types, tests clean | Partial | `npm run build`, `npm run lint`, `npx tsc --noEmit --pretty false`, and `npm run secrets:scan` passed on May 6 after the copy, identity, live-schema, Next project-root, markdown, Sentry, and dependency cleanup. `npm run build` and `npx next start` no longer emit the previous multiple-lockfile workspace-root warning. `npm test` now runs the launch-focused unit suite and passed with 88 Node tests plus 87 Vitest tests. E2E remains separate because it requires a production-style server plus seeded/provider-backed test users. |
| 2. Playwright N2/N3/N4 green | Partial | N2 paywall passed a fresh `10/10` on May 6 after reseeding the fixed E2E personas and aligning the quick-take cap test with duplicate-completion idempotency; N3 discussions also passed a fresh `10/10` on May 6; narrowed N4 auth passed signup/login/forgot-password `3/3` fresh on May 6. Full N4 remains out of launch scope per owner direction. |
...
| 5. Plan limits + throttle | Partial | Paywall N2 coverage passed fresh on May 6. User-triggered grading paths now reserve `ai_grading_runs` and pass AI spend budgets for FLOW elaboration/freeform submit, coding submit/finalize, interview challenge submit, and interview-loop debrief; duplicate FLOW step retries return saved answers before quota reservation. Full live per-route AI throttle/limit E2E across every AI route is still not recorded. |
...
| 8. Stripe | Partial | Paywall checkout/webhook scenarios passed in the prior launch gate, and AI grading routes now use plan/budget enforcement. Affiliate mechanics are disabled across routes, auth attribution, checkout discounting, webhook commissions, and payout cron unless `NEXT_PUBLIC_ENABLE_AFFILIATES=true`. Affiliate real signup smoke is still blocked by Stripe Connect and missing affiliate env vars. Owner/provider setup required. |
...
| 12. Legal/help | Local complete, production blocked | Local production smoke returned `200` for `/privacy`, `/terms`, `/pricing`, `/help`, and `/changelog`. `PUBLIC_DIRECTORY_PATHS` includes those paths for sitemap generation. Live production currently serves an older mixed deployment where `/privacy`, `/terms`, `/help`, and `/changelog` resolve to login, so production page checks remain blocked until deployment/domain routing is updated. |
...
| 18. Sentry receiving | Partial | Code-side Sentry support is wired through `@sentry/nextjs`, `src/instrumentation.ts`, `src/instrumentation-client.ts`, server and edge config files, `src/app/global-error.tsx`, and CSP `connect-src` access for `*.sentry.io`. Source map upload stays disabled unless `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, and `SENTRY_PROJECT` are set. Runtime capture requires `NEXT_PUBLIC_SENTRY_DSN` or `SENTRY_DSN`. `npx tsc --noEmit --pretty false`, `npm run lint`, `npm run build`, `npm run secrets:scan`, and critical-level production dependency audit passed after the wiring. Provider receiving is still unverified until a production or staging event is observed in Sentry. |
...
| 20. PWA installable | Local complete, production blocked | Static/local production audit found `/manifest.json` public, valid, and backed by dedicated 192px and 512px PNG icons. Local Chromium CDP browser probe against `next start` returned `installabilityErrors: []`. Production installability remains blocked until the live domain serves the current app. See `docs/notes/pwa-manifest-audit.md`. |
...
| 22. Streak + XP correctness | Partial | `docs/notes/xp-streak-audit.md` now records a read-only audit plus narrow duplicate-award fixes. Repeated `/api/challenges/[id]/complete` calls for an already completed FLOW attempt and repeated quick-take submits for the same completed quick-take return stored feedback before XP/streak/move-level writes. Full sign-off still lacks an XP ledger, shared calculator, atomic XP increment, UTC boundary E2E, and simultaneous-completion coverage. |
...
| 24. Markdown rendering | Partial | Challenge scenario/context prose now renders markdown in `CaseContextPane`, `ChallengeWorkspace`, `FlowWorkspace`, challenge feedback, and orientation while preserving inherited text color via `Md tone="inherit"`. Evidence: `8d31e8a`, follow-up prompt render cleanup, `rg -n "\\{challenge\\.prompt_text\\}|challenge\\.prompt_text\\}" src/app src/components` returns only `Md` render sites, `npx vitest run tests/components/Md.test.tsx` passed `5/5`, and `npx tsc --noEmit --pretty false`, `npm run lint`, `npm run build`, and `npm run secrets:scan` passed. Hatch challenge-link E2E is still not recorded. |
...
- Production env, deployment, and provider checks remain owner-controlled: no E2E fallback flags, OpenAI, Turnstile, Upstash, status DNS/provider, public pages, manifest, health route, security headers.
