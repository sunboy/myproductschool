...
| 22. Streak + XP correctness | Partial | `docs/notes/xp-streak-audit.md` now records a read-only audit plus a narrow FLOW duplicate-finalization fix. Repeated `/api/challenges/[id]/complete` calls for an already completed attempt return stored feedback before XP/streak/move-level writes. Full sign-off still lacks an XP ledger, shared calculator, atomic XP increment, quick-take idempotency, UTC boundary E2E, and simultaneous-completion coverage. |
