...
This checkpoint supersedes earlier access and staging blockers below; historical browser results remain limited to the deployment and database on which they were observed.

- Candidate `471b7ada1ecad6ec1c45338f81b16e6a3c277ab7` is READY as `dpl_ECUyVi6Rh18RnXrwciLAm971HfRF` on the feature-branch preview. Landing and authentication changes from `feat/full-product-redesign` are integrated.
...
- Genuine Stripe TEST creation, duplicate resend, delayed update, and cancellation passed a clean lifecycle rerun on this candidate. Hosted and database state returned to Free with Pro access disabled and subscription status canceled. The dedicated test endpoint remains enabled. See [billing lifecycle evidence](../runbooks/staging-billing-lifecycle.md).
- Separate staging sandbox/gateway services, private snapshot storage, and scheduled cleanup are verified. A real gateway request recorded $0.00002; enforcement and key revocation passed. The next learner session is limited to a $0.49 gateway budget and 1800-second TTL. Complete analytics report, reusable skill, grade persistence, and learner cleanup remain open pending the genuine canary.
...
- Browser Checkout payment/redirect/portal, remaining deployed responsive checks, and actual spoken voice/debrief checks remain open. The Mac was locked at the last browser attempt; no browser or spoken result is inferred from API tests.
