...
Reactivation on `cf7e9fee` failed after successful password reauthentication. Vercel returned HTTP 500 because Stripe request `req_83iaFtD4bk29o3` received both `cancel_at_period_end` and `cancel_at`; Stripe rejected the request with `StripeInvalidRequestError` and supplied no `code` or `param`.

The corrected route passed on candidate `b01d65855e1620731a950cfe26a980b2e98ceb8b`, deployment `dpl_3Mhe4EHhhtsCVNW3Lcyj8tTm8oJh`. After password reauthentication, **Keep Pro** completed without the failure. Stripe reported `cancel_at=null`, `cancel_at_period_end=false`, and `canceled_at=null`; genuine update event `evt_1UCoGOEGJUB78L7nSZE68IZw` processed once with a cleared lease and no error; and the database plus authenticated hosted profile remained Pro. Reloaded Settings showed **Next billing** and **Cancel at renewal**. Evidence: [settings-reactivation-confirmed-b01d6585.png](./evidence/staging-20260906/settings-reactivation-confirmed-b01d6585.png).
...
The successful retry fixture was also cleaned up under exact ownership guards. Subscription `sub_1UCoE1EGJUB78L7nYcTOpach` is canceled; deletion event `evt_1UCoNAEGJUB78L7nored50ic` has `pending_webhooks=0` and processed once with a cleared lease and no error; the database and authenticated hosted profile are Free with `pro_access=false`; and the staging customer again has zero nonterminal subscriptions. Its verified pre-cleanup state is preserved mode `0600` at `.vercel/staging-settings-reactivate-retry-verified-b01d6585.json`.

Candidate `7e3c4d2822abee64560fc9db3d12734729618457`, deployment `dpl_4gzMyq6K7V9xeAxaS5C8qHfJECpS`, also passed the stricter first-read check. The exact TEST fixture began with explicit `cancel_at=1789338772`, `cancel_at_period_end=false`. Immediately after password confirmation and **Keep Pro**, Settings showed **Next billing** and **Cancel at renewal** without a reload. Stripe showed `cancel_at=null`, `cancel_at_period_end=false`; update event `evt_1UCoeaEGJUB78L7nzlA367xR` processed once with cleared lease/token and no error, and Pro mirrors were current. Evidence: [settings-immediate-reactivation-7e3c4d28.png](./evidence/staging-20260906/settings-immediate-reactivation-7e3c4d28.png).

Guarded cleanup canceled only TEST subscription `sub_1UCocKEGJUB78L7ntGwmPc7L`; deletion event `evt_1UCofSEGJUB78L7nvCAI6HQK` processed once. Final Stripe/database/hosted mirrors were Free/canceled with zero blocking subscriptions. Mode-`0600` evidence is preserved at `.vercel/staging-settings-reactivate-immediate-verified-7e3c4d28.json` and `.vercel/staging-settings-reactivate-immediate-cleanup-verified-7e3c4d28.json`.
