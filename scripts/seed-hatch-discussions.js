#!/usr/bin/env node
/**
 * seed-hatch-discussions.js
 *
 * Seeds every challenge with 1 Hatch expert discussion + 2 reply threads.
 * Uses Claude to generate contextually relevant content per challenge.
 * Idempotent: skips challenges that already have Hatch discussions.
 *
 * Requires migration 018_luma_seed_flag.sql to be applied first
 * (makes user_id nullable, adds display_name column).
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... node scripts/seed-hatch-discussions.js
 *   ANTHROPIC_API_KEY=sk-... node scripts/seed-hatch-discussions.js --limit=3
 *   ANTHROPIC_API_KEY=sk-... node scripts/seed-hatch-discussions.js --challenge=<id>
 */
...
if (!ANTHROPIC_API_KEY) {
  ...
  console.error('Usage: ANTHROPIC_API_KEY=sk-ant-... node scripts/seed-hatch-discussions.js')
  process.exit(1)
}
...
const HATCH_COACH_NAME = 'Hatch'
const HATCH_DATA_NAME  = 'Hatch · Data'
const HATCH_STRAT_NAME = 'Hatch · Strategy'
...
async function generateDiscussions(title, promptText) {
  const msg = await anthropic.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `You are Hatch, an AI product thinking coach. A new challenge was posted: "${title}".
...
async function main() {
  console.log('Seeding Hatch discussions...\n')
  ...
  // Check which already have Hatch expert discussions (idempotent guard)
  const { data: existing, error: existErr } = await supabase
    .from('challenge_discussions')
    .select('challenge_id')
    .eq('display_name', HATCH_COACH_NAME)
    .is('user_id', null)
  ...
  for (const challenge of toSeed) {
    ...
    try {
      ...
      // Insert expert discussion post (Hatch coach frame)
      const { data: discussion, error: discErr } = await supabase
        .from('challenge_discussions')
        .insert({
          challenge_id: challenge.id,
          user_id: null,
          display_name: HATCH_COACH_NAME,
      ...
      const replyRows = [
        {
          ...
          user_id: null,
          display_name: HATCH_DATA_NAME,
          content: content.data_reply,
        },
        {
          discussion_id: discussion.id,
          user_id: null,
          display_name: HATCH_STRAT_NAME,
