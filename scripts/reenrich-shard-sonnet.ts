...
const FAILURES_LOG = path.join(process.cwd(), 'seeds', 'reenrichment-sonnet-failures.log')
...
CORRECTNESS DISCIPLINE (the most important rule):
Before finalizing, mentally TRACE your reference_solution.python against EACH test case's args. Compute what the function will return step by step. The 'expected' value MUST exactly match what your solution returns. Common mistakes to avoid:
- Off-by-one counting (Coin Change combinations, parenthesis balancing)
- Forgetting to deduplicate (3Sum allowing same triplet from duplicate elements)
- In-place mutation: if your solution modifies args in place and returns None/undefined, set output to null/None and DON'T return the modified array — OR rewrite the solution to return a new array.
- Multiple valid answers: if there's any ambiguity in the output (which pair to pick, what order), set compare_mode appropriately.
- Tree/list edge cases: empty input ([]), single node, deeply skewed.
For every test case, ask: "if I run my solution on these args by hand, do I really get this expected value?" If unsure, simplify the test case to one you CAN verify.`
}
...
function shardOutPath(shardIdx: number): string {
  return path.join(process.cwd(), 'seeds', `reenrich-sonnet-shard-${shardIdx}.json`)
}
