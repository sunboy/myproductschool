...
const CHALLENGES: ChallengeSeed[] = [
  ...
  //     stays true; the upsert overwrites the broken metadata.
  {
    ...
    claudeMd: [
      ...
      '',
      'The `events` table holds 30 days of checkout events: `event_id`, `user_id`,',
      '`session_id`, `ts`, `event_name`, `device`, `region`. The `event_name` values',
      'in funnel order are `view_cart`, `start_checkout`, `enter_address`,',
      '`enter_payment`, `submit_order`, and `order_confirmed`.',
      '',
      ...
      'schema and the distinct `event_name` values before assuming the funnel order.',
      '',
      'Dataset: `hackproduct.case_001_checkout_funnel` (table `events`).',
    ].join('\n'),
    subProblems: [
      { id: 'explore_schema', objective: 'Inspect the events table and list the distinct `event_name` values to establish the funnel order.', successCriterion: 'Paste the column list and the distinct `event_name` values.', suggestedPrompts: ['Show the events schema and the distinct event_name values', 'Show a few sessions of events in ts order'] },
