...
interface AskLumaDrawerProps {
  open: boolean
  onClose: () => void
}

/** Render Luma markdown: bold, bullets, paragraphs. No external dependency. */
function LumaText({ text }: { text: string }) {
...
export function AskLumaDrawer({ open, onClose }: AskLumaDrawerProps) {
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-80 bg-background flex flex-col shadow-2xl border-l border-outline-variant">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-outline-variant/40 bg-primary-fixed">
          <LumaGlyph size={28} state={loading ? 'speaking' : 'idle'} className="shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-on-surface font-headline">Ask Luma</p>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {messages.length === 0 && (
            <div className="text-center py-8">
              <LumaGlyph size={48} state="listening" className="mx-auto mb-3" />
              <p className="text-sm font-semibold text-on-surface mb-1">Hi! I&apos;m Luma.</p>
          ...
          {messages.map((msg, i) => (
            ...
            >
              {msg.role === 'luma' && (
                <LumaGlyph size={22} state="speaking" className="shrink-0 mt-0.5" />
              )}
              <div
              ...
              >
                {msg.role === 'luma' ? <LumaText text={msg.content} /> : msg.content}
              </div>
            </div>
          ))}
