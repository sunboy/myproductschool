...
import type { LumaChatMessage } from '@/context/LumaContext'
...
function LumaText({ text, onNavigate }: { text: string; onNavigate: (href: string) => void }) {
...
export function FloatingLuma() {
  ...
  const glyphState = lumaCtx?.state ?? 'idle'

  // Chat messages live in context so they persist across page navigations
  const messages: LumaChatMessage[] = lumaCtx?.chatMessages ?? []
  const setMessages = lumaCtx?.setChatMessages ?? (() => {})
  ...
  // Listen for open-ask-luma event
  useEffect(() => {
    const handler = () => { setOpen(true); setBubble(false) }
    window.addEventListener('open-ask-luma', handler)
    return () => window.removeEventListener('open-ask-luma', handler)
  }, [])
  ...
  const sendMessage = useCallback(async () => {
    ...
    const userMsg: LumaChatMessage = { role: 'user', content: text }
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'luma', content: reply }])
    } catch {
      setMessages(prev => [...prev, { role: 'luma', content: "I'm having trouble responding right now. Try again in a moment." }])
    } finally {
      setLoading(false)
    }
  ...
  const contextMessage = (lumaCtx?.message && lumaCtx.message.length > 0)
    ? lumaCtx.message
    ...
    <div className={`fixed right-5 z-40 flex flex-col items-end gap-2 pointer-events-none ${isWorkspace ? 'bottom-20' : 'bottom-5'}`}>
      ...
      {open && (
        <div
          ...
          style={{
            ...
            animation: 'lumaSlideUp 0.2s cubic-bezier(0.34,1.56,0.64,1) both',
          }}
        >
          {/* Header */}
          <div
          ...
          >
            <LumaGlyph size={28} state={loading ? 'speaking' : 'idle'} className="text-primary shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-on-surface font-headline leading-tight">Luma</p>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2.5" style={{ scrollbarWidth: 'none' }}>
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-2 pb-4">
                <LumaGlyph size={44} state="listening" className="text-primary" />
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-1.5 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                {msg.role === 'luma' && (
                  <LumaGlyph size={18} state="speaking" className="text-primary shrink-0 mt-0.5" />
                )}
                <div
                ...
                >
                  {msg.role === 'luma'
                    ? <LumaText text={msg.content} onNavigate={handleNavigate} />
                    : msg.content}
                </div>
              </div>
            ))}
      ...
      {showBubble && (
        <div
          ...
          style={{ maxWidth: 220, animation: 'lumaFadeUp 0.2s ease both' }}
          onClick={toggleOpen}
        >
          <div
      ...
      <button
        ...
        aria-label={open ? 'Close Luma' : 'Ask Luma'}
      >
        <LumaGlyph size={36} state={open ? 'listening' : glyphState} className="text-white" />
