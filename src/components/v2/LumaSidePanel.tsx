...
export interface LumaSidePanelProps {
  message: string
  // Fix D: added 'speaking' — appropriate for coaching panels
  lumaState: 'idle' | 'listening' | 'reviewing' | 'speaking'
  stepName?: string
}

export function LumaSidePanel({ message, lumaState, stepName }: LumaSidePanelProps) {
  ...
  return (
    <div className="bg-surface-container-low rounded-xl p-4 flex flex-col gap-3">
      {/* Header: Luma glyph + label */}
      <div className="flex items-center gap-2">
        <div ref={glyphRef} className="shrink-0">
          <LumaGlyph size={48} state={lumaState} className="text-primary" />
        </div>
        <div className="flex flex-col">
          <span className="font-label text-xs text-on-surface-variant leading-none">Luma</span>
          {stepName && (
