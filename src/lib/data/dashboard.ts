...
export async function getDashboardPreferences(userId: string): Promise<DashboardPreferences> {
  ...
  const { data } = await supabase
    .from('profiles')
    .select('dashboard_cards, dismissed_cards, interview_date, dashboard_card_sizes')
    .eq('id', userId)
    .single()
  ...
  return {
    ...
    dashboard_card_sizes: (data?.dashboard_card_sizes ?? {}) as Record<string, number>,
  }
}
...
export async function getUserInterviews(userId: string): Promise<UserInterview[]> {
  ...
  const { data } = await supabase
    ...
    .select('id, interview_date, company, round')
    .eq('user_id', userId)
    .gte('interview_date', today)
    .order('interview_date', { ascending: true })

  return (data ?? []) as UserInterview[]
}
