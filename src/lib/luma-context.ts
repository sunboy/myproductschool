...
function deriveOverallLevel(competencies: LumaUserContext['competencies']): LumaUserContext['overallLevel'] {
...
function deriveWeakestCompetency(competencies: LumaUserContext['competencies']): string | null {
...
export async function getLumaContext(userId: string): Promise<LumaUserContext> {
  try {
    ...
    const [
      ...
      // 7. luma_context — last 3 active insights
      (async () => {
        try {
          const { data } = await admin
            .from('luma_context')
    ...
    return {
      ...
      recurringPatterns: (failurePatternsResult as LumaUserContext['recurringPatterns']) ?? [],
      recentStepAttempts: (stepAttemptsResult as LumaUserContext['recentStepAttempts']) ?? [],
      recentCompletions: (completionsResult as LumaUserContext['recentCompletions']) ?? [],
      lumaInsights: (lumaInsightsResult as string[]) ?? [],
    }
  } catch {
...
function isEmptyContext(ctx: LumaUserContext): boolean {
  return (
    ...
    ctx.lumaInsights.length === 0
  )
}

export function buildLumaContextString(
  ctx: LumaUserContext,
