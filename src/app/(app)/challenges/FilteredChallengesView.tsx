...
const ALL_VIEW_DISCIPLINES = ['product_sense', 'system_design', 'data_modeling', 'sql', 'algorithm'] as const
...
function challengeMatchesDiscipline(type: string | null | undefined, discipline: Discipline) {
  ...
  if (discipline === 'product_sense') return ['flow', 'freeform', 'quick_take', 'claude_code_analytics'].includes(type ?? '')
...
export function FilteredChallengesView({
  ...
  const totalForDiscipline = counts[discipline as CountDiscipline] ?? 0
      ...
      <section data-tour-target="practice-filters" className="px-4 pb-4 sm:px-6">
        <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-6">
          {DISCIPLINES.map((entry) => {
            ...
            const count = counts[entry.key as CountDiscipline] ?? 0
...
/* ──────────────────────────────────────────────────────────────────────────
 * "All practice" overview — five discipline preview sections (capped), each
 * with a "see all N" link. Preview rows come from the SSR seed; each section
 * can "load more" via /api/challenges without leaving the overview.
 * ────────────────────────────────────────────────────────────────────────── */
function AllPracticeView({
  ...
  const visibleDisciplines = ALL_VIEW_DISCIPLINES.filter((d) => (counts[d as CountDiscipline] ?? 0) > 0)
    ...
    <div className="flex flex-col gap-8">
      {visibleDisciplines.map((disc) => (
        <AllPracticeSection
          ...
          total={counts[disc as CountDiscipline] ?? 0}
          seed={seeded[disc] ?? []}
          filters={filters}
          paradigms={paradigms}
          summaries={summaries}
