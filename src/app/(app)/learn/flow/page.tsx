...
const MOVES: MoveData[] = [
  {
    ...
    cards: [
      ...
      },
      {
        ...
        body: 'Frame is the move where you decide what problem you\'re actually solving. Most people skip it — they jump straight to solutions before questioning the brief. Strong framers slow down, challenge assumptions, and redefine the problem so the solution space becomes clearer.',
  ...
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        body: 'A well-structured List prevents you from solving the loudest part of a problem while the real driver hides in a corner. Different user segments, stakeholders, or use cases often have fundamentally different needs — and deserve separate rows.',
        highlight: 'Map the whole population before drawing conclusions.',
      },
      {
        title: 'List in the wild',
        body: exampleBody('Power users vs. new users react differently to the same change.', 'Bad list: "Users, admins, people who pay." Good list: "New users (0-7 days) · Active users (weekly) · Power users (daily) · Churned (30d+ inactive)." The second is structured — each bucket is distinct and the four together cover everyone.'),
      },
    ],
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        body: 'Optimize is where you sharpen from many options to the best one given real constraints — time, resources, risk, and evidence. It\'s not about the perfect option in a vacuum. It\'s about the best defensible call with what you actually have.',
        highlight: 'A perfect option you can\'t ship isn\'t an option.',
      },
      {
        title: 'The recommendation, not the menu',
        body: 'Optimize is what separates PMs who produce clarity from PMs who produce decks full of bullets. Your job is to make the call — and to make it legible to others. Validate the riskiest assumptions first, then commit.',
        highlight: 'Come with a PoV before the meeting, not during.',
      },
      {
        title: 'Optimize in the wild',
        body: exampleBody('Cut free tier from 10K to 1K API calls — or find a smarter lever?', 'Bad optimize: "We should cut the free tier, or maybe an add-on, or maybe a pricing study." Good optimize: "Cutting to 1K is highest-leverage — triggers 40% of power free-users to evaluate paid. Validate with a 2-week shadow limit before enforcement." The second makes a call.'),
      },
    ],
  },
  {
    ...
    cards: [
      {
        ...
        body: 'You can Frame the problem perfectly, List every option, and pick the right one — and still fail if you can\'t land it with the people who need to act on it. Win is the move that makes the other three matter.',
        highlight: 'Clarity is a product deliverable.',
      },
      {
        title: 'BLUF: recommendation first, evidence after',
        body: 'Win is the move where you land your recommendation with clarity and conviction. Structure your argument so the most important thing lands first. Bottom Line Up Front — the room knows what you\'re recommending before you\'ve shown a single data point.',
      ...
      },
      {
        ...
        body: 'It\'s not just communication polish — it\'s handling pushback without conceding the direction. The Win move is absorbing a constraint and redirecting it into the plan, not fighting it or folding.',
        highlight: 'A brilliant product call that can\'t be communicated isn\'t a product call.',
      },
      {
        title: 'Win in the wild',
        body: exampleBody('Convince your CEO to invest in auth refactor over 3 features.', '"Auth refactor is the safer bet this quarter. Three features assume the current auth holds — it won\'t past 50K users. We\'d ship features onto a cracking foundation. Refactor now unblocks all three features in Q2 with half the risk." Lead with the recommendation, then the evidence.'),
      },
    ],
  },
]
...
const WORKED = [
  {
    ...
    body: 'The question isn\'t "which feature?" — it\'s why daily active listening dropped among 18-24s. Discovery data shows choice paralysis: too many options, no clear starting point. The real job is reducing friction-to-first-listen, not adding more surfaces. "Build AI DJ vs social feed" was never the right question.',
    chips: ['JTBD reframe', 'Not "which feature"', 'Discovery data'],
  },
  {
    symbol: 'L',
    title: 'List — Open the space',
    body: 'Solutions generated: AI DJ, social feed, contextual playlists (morning/gym/focus), "continue from friends" widget, mood-based radio, one-tap start from home screen. That\'s six options — not two. The original framing collapsed the solution space prematurely. Contextual playlists and one-tap start weren\'t even on anyone\'s radar.',
    chips: ['6 options', 'Opportunity tree', 'Expanded framing'],
  },
  {
    symbol: 'O',
    title: 'Optimize — Score with evidence',
    body: 'RICE: AI DJ scores 94 — high reach, proven listen-through rate in beta, medium effort. Social feed scores 41 — low confidence, failed twice before in different form. Contextual playlists score 67 — high reach, lower novelty. One-tap start scores 58 — low effort but limited sustained impact. AI DJ wins clearly, with data behind it.',
    chips: ['RICE scoring', 'Beta data', 'AI DJ wins at 94'],
  },
  {
    symbol: 'W',
    title: 'Win — Position to get buy-in',
    body: 'Not: "We should build AI DJ (RICE: 94) instead of the social feed." Instead: "For our Gen Z listeners who stall on the home screen, the AI DJ replaces the decision they currently dread — not the social feed they don\'t use. Beta cohort shows 23% lift in listen-through. We can prove Q3 impact in a 4-week sprint." One read, clear yes.',
...
// ── Inline story cards ─────────────────────────────────────────
function InlineStoryCards({ move, open }: { move: MoveData; open: boolean }) {
  ...
  function goCard(dir: 'forward' | 'back') {
    setAnimDir(dir)
    setAnimating(true)
    setTimeout(() => {
      setCardIndex(prev => dir === 'forward' ? prev + 1 : prev - 1)
      setAnimating(false)
    }, 180)
  }

  const card = move.cards[cardIndex]
  const isLast = cardIndex === move.cards.length - 1
  const moveIndex = MOVES.findIndex(m => m.key === move.key)
  const nextMove = moveIndex < MOVES.length - 1 ? MOVES[moveIndex + 1] : null

  const transitionClass = animating
    ? animDir === 'forward' ? 'opacity-0 translate-x-3' : 'opacity-0 -translate-x-3'
      ...
      <div className="pt-4 pb-2">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="font-label text-xs font-bold uppercase tracking-widest text-on-surface-variant">
              {move.label} — story {cardIndex + 1} of {move.cards.length}
            </span>
          </div>
          <ProgressDots total={move.cards.length} current={cardIndex} color={move.accentColor} />
        </div>

        {/* Card */}
        <div className={`rounded-2xl overflow-hidden transition-all duration-180 ${transitionClass}`} style={{ background: move.bgColor }}>
          <div className="h-1 w-full" style={{ background: move.accentColor, opacity: 0.6 }} />
          <div className="p-5 space-y-3">
            <h3 className="font-headline text-lg font-bold text-on-surface">{card.title}</h3>
            {card.body.split('\n\n').map((para, i) => (
              <p key={i} className="text-sm text-on-surface leading-relaxed whitespace-pre-line">{para}</p>
            ))}
            {card.highlight && (
              <div className="rounded-xl px-4 py-3 border-l-4" style={{ background: move.accentColor + '18', borderColor: move.accentColor }}>
                <p className="text-sm font-bold" style={{ color: move.accentColor }}>{card.highlight}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-4">
          <button
            ...
            className="flex items-center gap-1 text-sm font-label font-bold text-on-surface-variant hover:text-on-surface disabled:opacity-0 transition-all"
          >
          ...
          {isLast && nextMove ? (
            <span className="text-xs font-label text-on-surface-variant">
              Next: scroll to {nextMove.label} →
            </span>
          ) : isLast ? (
            <Link
              href="/challenges"
              className="inline-flex items-center gap-1 bg-primary text-on-primary rounded-full px-4 py-1.5 font-label font-bold text-sm hover:opacity-90 transition-opacity"
            >
              Practice challenges
            ...
            <button
              onClick={() => goCard('forward')}
              className="flex items-center gap-1 text-sm font-label font-bold transition-colors"
              style={{ color: move.accentColor }}
            >
              Next
...
function LumaRail({ message, visible }: { message: string; visible: boolean }) {
  return (
    <div
      className="sticky top-20 hidden lg:block transition-all duration-400"
...
export default function LearnFlowPage() {
  ...
  const [openStory, setOpenStory] = useState<FlowMove | null>(null)
  const [openWorked, setOpenWorked] = useState<number | null>(null)
  const sectionRefs = useRef<Map<string, HTMLElement>>(new Map())

  // Scroll reveals + Luma rail visibility + active pill
  useEffect(() => {
  ...
  const PILL_LABELS = [
    { key: 'frame' as FlowMove, label: 'F — Frame' },
    { key: 'list' as FlowMove, label: 'L — List' },
    { key: 'optimize' as FlowMove, label: 'O — Optimize' },
    { key: 'win' as FlowMove, label: 'W — Win' },
  ]
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex items-start gap-4 mt-6 mb-5">
          <LumaGlyph size={44} state="speaking" className="text-primary shrink-0 mt-1" />
          <div>
        ...
        <div className="flex gap-2 flex-wrap mb-6">
          {PILL_LABELS.map(p => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full font-label text-xs font-bold transition-all duration-200 border ${
                activePill === p.key
        ...
        <div className="flex gap-3 flex-wrap">
          <Link href="/challenges" className="inline-flex items-center gap-2 bg-primary text-on-primary rounded-full px-5 py-2.5 font-label font-bold text-sm hover:opacity-90 transition-opacity">
            Try a FLOW challenge
            <span className="material-symbols-outlined text-base" style={{ fontVariationSettings: "'FILL' 0" }}>arrow_forward</span>
          </Link>
          <button
            onClick={() => scrollTo('frame')}
            className="inline-flex items-center gap-2 border border-outline-variant text-on-surface rounded-full px-5 py-2.5 font-label font-bold text-sm hover:bg-surface-container transition-colors"
          >
            Read the framework
          </button>
        </div>
          ...
          >
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-8 items-start">
              ...
              <div>
                ...
                >
                  <button
                    onClick={() => setOpenStory(openStory === move.key ? null : move.key)}
                    className="inline-flex items-center gap-2 rounded-full px-5 py-2 font-label font-bold text-sm transition-all border-2"
                    style={{
                      background: openStory === move.key ? move.accentColor : 'transparent',
                      color: openStory === move.key ? '#fff' : move.accentColor,
                      borderColor: move.accentColor,
                    }}
                  >
                    <span className="material-symbols-outlined text-base" style={{ fontVariationSettings: "'FILL' 1" }}>
                      {openStory === move.key ? 'close' : 'play_arrow'}
                    </span>
                    {openStory === move.key ? `Close ${move.label} walkthrough` : `Try ${move.label} →`}
                  </button>

                  <InlineStoryCards move={move} open={openStory === move.key} />
                </div>
              </div>
      ...
      <div className="py-20 px-6 text-center">
        <div className="max-w-lg mx-auto">
          ...
          <div className="flex gap-3 justify-center flex-wrap mb-4">
            ...
            <Link
              href="/calibration"
              className="inline-flex items-center gap-2 border border-outline-variant text-on-surface rounded-full px-6 py-3 font-label font-bold text-sm hover:bg-surface-container transition-colors"
            >
              Take calibration
            </Link>
          </div>
