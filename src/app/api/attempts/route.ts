...
export async function GET(req: NextRequest) {
  ...
  let query = admin
    ...
    .select('id, challenge_id, grade_label, total_score, max_score, completed_at, feedback_json, canvas_png_url, canvas_final_snapshot, challenges(title, challenge_type)')
    .eq('user_id', user.id)
    .eq('status', 'completed')
  ...
  const attempts = rows.map((row: any) => {
    ...
    return {
      ...
      canvas_final_snapshot: (row.canvas_final_snapshot as Record<string, unknown> | null) ?? null,
    }
  })
